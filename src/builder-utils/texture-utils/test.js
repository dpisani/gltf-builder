...
import { buildTextureFromArrayBuffer } from './unwired';
...
describe('buildTextureFromArrayBuffer utility', () => {
  ...
  describe('texture', () => {
    ...
    beforeEach(() => {
      texture = buildTextureFromArrayBuffer(imageBuffer, 'image/png', deps)
        .texture;
    });

    it('sets a source image on the texture', () => {
      texture.source.should.be.calledOnce();
      texture.source.firstCall.args[0].should.be.an.instanceOf(TextureImage);
    });

    describe('textureImage', () => {
      let textureImage;
      beforeEach(() => {
        textureImage = texture.source.firstCall.args[0];
      });
      ...
      it('sets a BufferView on the image', () => {
        ...
        textureImage.bufferView.firstCall.args[0].should.be.an.instanceOf(
          BufferView
        );
      });
      ...
      describe('bufferView', () => {
        let bufferView;
        beforeEach(() => {
          bufferView = textureImage.bufferView.firstCall.args[0];
        });
